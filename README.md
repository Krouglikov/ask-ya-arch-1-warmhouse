...
Введены микросервисы регистрации (`apps/registration`), управления (`apps/control`) и представлений (`apps/presentation`).

Из их числа реализованы только сервис регистрации (Kotlin) и управления (Java); 
сервис представлений за недостатком времени оставлен в виде начального Spring-приложения без функционала.

Сервис регистрации интегрирован с монолитом в части добавления (регистрации) устройств через REST-API. 
В настоящий момент он просто передает входящий запрос на эндпойнт монолита без изменений в структуре запроса.
В перспективе сервис должен быть расширен и адаптирован для самостоятельной работы без использования монолита.

Сервис управления интегрирован с монолитом в части запросов к устройствам через REST-API.
В настоящее время он принимает запросы от монолита и направляет их без изменения структуры на сервис эмуляции температуры.
В перспективе сервис должен расширяться в части опроса разнообразных датчиков и/или получения от них данных через очередь сообщений.
Кроме того, он должен будет самостоятельно записывать показания в БД и вырабатывать управляющие воздействия.

Для тестирования введенных сервисов создана Postman-коллекция запросов `apps/microservice-testing.postman_collection.json`.
- `temperature-api: get sensor A` -- запрос показаний сенсора из сервиса-эмулятора датчиков температуры;  
- `registration: add user` -- регистрация нового пользователя сервисом регистрации (реализовано заглушкой);
- `registration: add device` -- регистрация нового устройства сервисом регистрации (реализовано направлением запроса в монолит);
- `control: get sensor A` -- запрос показаний сенсора сервисом управления (реализовано направлением запроса в сервис-эмулятор);

Для проверки интеграции монолита с сервисом управления нужно воспользоваться имеющейся Postman-коллекцией `apps/smarthome-api.postman_collection.json`.
Запросы показаний температуры теперь вызывают сервис управления, а не сервис-эмулятор напрямую как ранее. 